...
const sort = (currentSortType) => {
  ...
  `<div class="trip-sort__item  trip-sort__item--${it}"><input id="sort-${it}" class="trip-sort__input  visually-hidden" type="radio" name="trip-sort" value="sort-${it}" data-sort-type= ${SortType[it]} ${currentSortType === SortType[it] ? 'checked' : ''}><label class="trip-sort__btn" ${it === 'offers' || it === 'event' ? 'disabled' : ''} for="sort-${it}">${it}</label></div>`);
  console.log(sortList(sortWords).join(' '));
  console.log(SortType['time']);

  return `<form class="trip-events__trip-sort  trip-sort" action="#" method="get">
<div class="trip-sort__item  trip-sort__item--day">
  <input id="sort-day" class="trip-sort__input  visually-hidden" type="radio" name="trip-sort" value="sort-day" checked data-sort-type="${SortType.date}"${currentSortType === SortType.date ? 'checked' : ''}>
